...
library(pacman)
p_load(tidyverse, phyloseq, permute, lattice, vegan, cowplot, 
       ggpubr, rstatix, car, RColorBrewer, ggdist)
...
R.S.S.index.long <- alpha.diversity.metrics.df %>% 
  pivot_longer(cols = c(Richness, ShannonIndex, expShannonIndex, SimpsonIndex, InverseSimpsonIndex),
               names_to = "DiversityMeasure", # une colonne pour le nom de l'indice
               values_to = "DiversityIndex") # une colonne pour la valeur liée à l'indice
...
plot.1.general <- ggplot(R.S.S.index.normal, aes(x = time, y = DiversityIndex)) +
  ...
  scale_color_brewer(palette = "Dark2"); plot.1.general
...
plot.2.general.box <- ggplot(R.S.S.index.hill, aes(x = DiversityMeasure, y = DiversityIndex)) +
  ...
  scale_color_brewer(palette = "Dark2"); plot.2.general.box
...
plot.3.bio <- ggplot(R.S.S.index.normal, aes(x = time, y = DiversityIndex, color = practice)) +
  ...
  scale_color_brewer(palette = "Dark2"); plot.3.bio
