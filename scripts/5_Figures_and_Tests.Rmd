...
ps <- readRDS("2023/out/ps_ITS_rarefied.rds")
...
## Figure 1A (PCoA)
```{r}
fill_practice <- c("#F28E2B", "#499894")
length(which(pcoa.df.9$practice=="Organic"))
length(which(pcoa.df.9$practice=="Conventional"))

(plot9 <- pcoa.df.9 %>% 
    ggplot(aes(x = PCo1, y = PCo2, colour = practice, fill = practice)) +
    geom_point(size = 2.5, aes(shape = code)) +
    theme_bw()+
    ...
    scale_fill_manual(name = "Management", values = fill_practice)+
...
ps.melt_agg <- left_join(ps.melt, taxonomie_agregee, 
  ...
  group_by(Sample, aggTaxo, practice) %>% # sum all Others into one by sample
...
b <- ps.melt_agg %>% 
  group_by(practice, aggTaxo) %>% 
  summarise(Abundance = sum(Abundance), .groups = 'drop') %>% 
  ggplot(aes(x = practice, y = Abundance, fill = aggTaxo)) +
  geom_bar(stat = 'identity', 
           position = 'fill')+
  facet_grid(~practice, scales = 'free')+
...
abun.summary.flower <- ps.melt_agg %>% 
  group_by(aggTaxo, practice) %>% 
  summarise(mean = mean(Abundance*100))%>%
  pivot_wider(names_from = practice, values_from = mean, names_prefix = "mean_");abun.summary.flower

arrange(abun.summary.flower, mean_Conventional)
arrange(abun.summary.flower, mean_Organic)

sum(abun.summary.flower$mean_Conventional)
sum(abun.summary.flower$mean_Organic)
...
mean(meta.flower$alphadiv[meta.flower$practice == "Conventional"])
mean(meta.flower$alphadiv[meta.flower$practice == "Organic"])
# Perform Shapiro-Wilk test for normality
meta.flower %>%
  shapiro_test(alphadiv) # Not normal

will<-wilcox.test(alphadiv ~ practice, data = meta.flower,
                  exact = FALSE)
...
plot.diversity.flower <- meta.flower %>% 
  ggplot(aes(practice, alphadiv, fill = practice))+ # What we want to plot
  ...
  scale_fill_manual(name = "Practice", values = fill_practice)+ # Colors according to the cultivar
  ...
  theme(axis.title.y = element_text(size =14),
        ...
        legend.position = "none");plot.diversity.flower
```
...
ggsave("final/submitted_figures_round2/Figure_1.pdf",
       bg = 'white', width = 2100, height = 2100,
       units = 'px', dpi = 200)
...
(plot1B <- pcoa.df.1 %>% 
   ...
   stat_ellipse( aes(group = practice),
                 level=0.95, geom = "polygon",alpha = 1/6, color = "black")+
...
```{r}
color_practice <- c("black", "black")
plot6 <-pcoa.df.6 %>% 
  ggplot(aes(x = PCo1, y = PCo2, colour = practice, fill = practice, shape = practice)) +
  geom_point(size = 2.5) +
  scale_shape_manual(name = "Management", values = c(21,21), aes(size = c(3, 0.1)))+
  scale_color_manual(values = color_practice)+
  scale_fill_manual(name = "Management", values = fill_practice)+
...
plot7 <-pcoa.df.7 %>% 
  ggplot(aes(x = PCo1, y = PCo2, colour = practice, fill = practice, shape = practice)) +
  geom_point(size=2.5) +
  scale_shape_manual(name = "Management", values = c(22,22))+
  scale_color_manual(values = color_practice)+
  scale_fill_manual(name = "Management", values = fill_practice)+
...
plot8 <-pcoa.df.8 %>% 
  ggplot(aes(x = PCo1, y = PCo2, colour = practice, fill = practice, shape = practice)) +
  geom_point(size=2.5) +
  scale_shape_manual(name = "Management", values = c(24,24))+
  scale_color_manual(values = color_practice)+
  scale_fill_manual(name = "Management", values = fill_practice)+
...
```{r}
fill_highlight <- c("#E1E1E0","#F28E2B", "#499894")
color_highlight <- c("#E1E1E0","black","black")
# Create a column that tell which site to highlight
# Highlight = D1 and D2
plot6B <- pcoa.df.6 %>%
  mutate(highlight = ifelse(code %in% c("D1", "D2"), code, "Other"),
         highlight = factor(highlight, levels=c("Other", "D1", "D2")))%>%
  arrange(highlight)%>%
  ggplot(aes(x = PCo1, y = PCo2, color = highlight, fill = highlight, shape = highlight))+
  geom_point(size=2.5)+
  theme_bw()+
  scale_color_manual(values = color_highlight)+
  scale_shape_manual(values = c(21,21,21))+
  # D1 and D2 will have color and the "other" will be gray
  scale_fill_manual(name = "Sites", values = fill_highlight)+
  labs(x = paste("PCo1:", 100*eig6[1], "%"),
       y = paste("PCo2:", 100*eig6[2], "%")) +
...
plot6C <- pcoa.df.6 %>%
  mutate(highlight2 = ifelse(code %in% c("B1", "B2"), code, "Other"),
         highlight2 = factor(highlight2, levels=c("Other", "B1", "B2")))%>%
  arrange(highlight2)%>%
  ggplot(aes(x = PCo1, y = PCo2, color = highlight2, fill = highlight2, shape = highlight2))+
  geom_point(size=2.5)+
  theme_bw()+
  scale_color_manual(values = color_highlight)+
  scale_shape_manual(values = c(21,21,21))+
...
ggsave("final/submitted_figures_round2/Figure_3.pdf",
       bg = 'white', width = 1152, height = 576,
       units = 'px', dpi = 65)
...
ps.melt_agg <- left_join(ps.melt, taxonomie_agregee, 
                         by = taxRank) %>% 
  mutate(aggTaxo = factor(aggTaxo, levels = taxLvl))%>% # reorder factor levels
  group_by(Sample, aggTaxo, practice, time) %>% # sum all Others into one by sample
  summarise(Abundance = sum(Abundance)) 


# simple stacked barplot :
c <- ps.melt_agg %>% 
  group_by(practice, time, aggTaxo) %>% 
  summarise(Abundance = sum(Abundance), .groups = 'drop') %>% 
  ggplot(aes(x = practice, y = Abundance, fill = aggTaxo)) +
  geom_bar(stat = 'identity', 
           position = 'fill')+
  facet_nested(~practice+time, scales = 'free')+
...
ggsave("final/submitted_figures_round2/Figure_4.pdf",
       bg = 'white', width = 2100, height = 2100,
       units = 'px', dpi = 200)


# mean abundance by taxa, by some metadata:
abun.summary.time <- ps.melt_agg %>% 
  group_by(aggTaxo, practice, time) %>% 
  summarise(mean = mean(Abundance*100))%>%
  pivot_wider(names_from = practice, values_from = mean, names_prefix = "mean_");abun.summary.time

# saveRDS(abun.summary.time, "/Users/bous2941/Documents/DOCTORAT/summary.leaf.relabun.time")
...
# saveRDS(abun.summary, "/Users/bous2941/Documents/DOCTORAT/summary.leaf.relabun")
```
...
plot.diversity <- ggplot(meta.leaf, aes(practice, alphadiv))+ # What we want to plot
  ...
  geom_boxplot(
    aes(fill = practice), 
    alpha = 1, width = 0.2, position = position_identity(), outliers = FALSE)+
  # Colors according to the cultivar
  scale_fill_manual(name = "Management", values = fill_practice)+ 
  # Seperate the plot according to the site and cultivar
  facet_wrap(~time)+
  guides(alpha = "none")+ # Remove size legend
...
plot.diversity2 <- meta.leaf %>% 
  ggplot(aes(time, alphadiv))+ # What we want to plot
  theme_bw()+ # General style of the plot (black & white)
  geom_violin(
    aes(fill = practice),
    alpha = 0.5,position = position_identity()) +
  ...
  theme(
    ...
    axis.title.y = element_text(size =16), # Axis text
    axis.text = element_text(size = 14)) +
  ylab("Shannon Index") +
  stat_pvalue_manual(stat.test.all, step.group.by = "code", y.position = 5.5, step.increase = 0.1, hide.ns = T);plot.diversity2
```
...
plot.diversity3 <- ggplot(meta.leaf, aes(time, alphadiv))+ # What we want to plot
  ...
  theme(
    ...
    axis.text = element_text(size = 14))+
  ylab("Shannon Index")+ # Y axis labels
  stat_pvalue_manual(stat.test, step.group.by = "practice", y.position = 5.5, step.increase = 0.1, hide.ns = T);plot.diversity3
```
...
ggsave("final/submitted_figures_round2/Figure_6.pdf",
       bg = 'white', width = 3721, height = 2117,
       units = 'px', dpi = 260)
...
mean(meta.flower.supp$alphadiv[meta.flower.supp$practice == "Conventional"])
mean(meta.flower.supp$alphadiv[meta.flower.supp$practice == "Organic"])

# Perform Shapiro-Wilk test for normality
meta.flower.supp %>%
  shapiro_test(alphadiv) # Not normal

wilcox.test(alphadiv ~ practice, data = meta.flower.supp,
            exact = FALSE)
...
stat.test.flower <- stat.diversity.flower.supp %>% add_xy_position(x = "practice") 
...
c_supp <- ps.melt_agg %>% 
  ...
  labs(fill = "Genus");c_supp


ggsave("final/submitted_figures_round2/Figure_S4.pdf",
       bg = 'white', width = 2100, height = 2100,
       units = 'px', dpi = 200)
...
plot.diversity4 <- ggplot(meta.leaf2, aes(practice, alphadiv))+ # What we want to plot
  ...
  ylim(0,7)+
  # Add statistical results on the plot
  stat_pvalue_manual(stat.test.leaf2, y.position = 5.5, step.group.by = "time", step.increase = 0.1, hide.ns = T);plot.diversity4
```
...
plot.diversity5 <- ggplot(meta.leaf2, aes(time, alphadiv))+ # What we want to plot
  ...
  guides(alpha = "none")+ # Remove size legendhttp://127.0.0.1:12599/graphics/plot_zoom_png?width=795&height=879
  theme(
    legend.position=c(0.999,0.99), legend.justification = c(1,1),
    legend.background = element_rect(
      fill="white",
      size=0.5, linetype="solid", 
      colour ="black"),
  ...
  ylab("Shannon Index")+ # Y axis labels
  ylim(0,6.5)+
  stat_pvalue_manual(stat.test.all2, step.group.by = "code", y.position = 5.5, step.increase = 0.1, hide.ns = T);plot.diversity5
```
...
plot.diversity6 <- ggplot(meta.leaf2, aes(time, alphadiv))+ # What we want to plot
  ...
  ylim(0,7) +
  stat_pvalue_manual(
    stat.test2, step.group.by = "practice", 
    y.position = 5.5, 
    step.increase = 0.1, 
    hide.ns = T);plot.diversity6
```
...
ggsave("final/submitted_figures_round2/Figure_S5.pdf",
       bg = 'white', width = 3721, height = 2117,
       units = 'px', dpi = 260)
```
